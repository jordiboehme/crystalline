...
House format: a motto as the first line (an H1, from v0.18.4 on), one
summary line, then `New & Noteworthy`, `Fixes` and `Breaking changes`
sections as they apply. No em or en dashes, plain '-' instead. Public text
only - nothing an agent session left behind.
