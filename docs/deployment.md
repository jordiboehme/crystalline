...
- **From the profile.** A signed-in account presses "Link" on the SSO identity card, which is a whole-page navigation to `GET /api/v1/auth/oidc/login?link=true`. The sign-on runs as usual and the callback ties the identity to the account that started it. The account that started the link has to be the account whose session finishes it: a callback arriving on no live session is refused with 401 (sign in again and start over), one arriving on another account's session with 409, and an identity another account already holds with a 409 that never says which account that is - an operator can move it, and a sign-on must not become a probe for who holds an identity here. The same card lists what is linked (`GET /api/v1/me/identity-links`) and gives one up (`DELETE /api/v1/me/identity-links/{issuer}`, the issuer percent-encoded as one path segment).
