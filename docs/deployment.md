...
Unregistering a domain stops it being served the moment it happens, whatever became of its index rows: nothing answers for a domain nobody registered, so the knowledge you deliberately removed cannot outrank what you kept. What is left after that is only disk, and the daemon tidies it in the background rather than leaving it there forever. Once an hour it checks every domain its index knows against the current configuration and stamps each one still registered, so it always has evidence of when a domain was last seen; a domain that has gone seven days without being registered has its engram rows collected, with its domain row left in place exactly as an explicit `domain remove` leaves it. Seven days give a `config.yaml` typo or a machine left off for a week room to recover before anything is dropped, and since nothing is served from those rows regardless, the only thing the wait costs is disk. A domain the sweep cannot be sure about is left alone rather than guessed at: a virtual domain's engrams are its only copy and are never swept away unattended (end one on purpose with `crystalline domain remove <domain> --purge`, which asks first), a domain another instance is actively hosting over a shared database stays that instance's to keep, and a read-only instance collects nothing at all. `crystalline doctor` reports what the sweep would collect and how long each candidate has gone unregistered, and `crystalline doctor --fix` collects them right away, grace period ignored - the immediate path for someone who just removed a domain on purpose and does not want to wait the week out.
...
A binary built without the bundle says so rather than pretending: the startup line reads `web UI not built into this binary`, and a navigation gets a `503` page naming `pnpm --dir fluid build` while the JSON API and MCP carry on unaffected. That is the state of a clone built without node, or of a `--no-default-features` build; every release binary carries the bundle, and so does the container image, which repackages the same binary. One combination is unsupported on such a build: OAuth for MCP clients needs the consent page, which lives in the bundle, so set `auth.oauth: false` there. It is otherwise derived on wherever `auth.mcp` is on and `service.ui` has not been turned off, and a start in that state prints a warning naming the key - nothing is granted, but a client that tries to connect is sent to a consent page that is not there.
...
The unit's own `ExecStart` carries no bind address of its own, and it stays
that way in any unit file an operator writes: an agent's `crystalline mcp`
connection can start a daemon of its own on this same machine, and a daemon
that did not read the same configuration would bind loopback while the LAN
endpoint the unit exists for quietly disappears. Exposure belongs to
configuration, not to a flag on one process's command line.
...
untouched. The shipped unit also carries `RestartPreventExitStatus=3` beside
its `Restart=on-failure`: exit code 3 means another Crystalline instance
already owns the index, which a restart cannot fix, so the unit stays
`failed` there instead of flapping between active and failed where
monitoring cannot see it. Carry that line into a hand-written unit too.
...
- `crystalline-data` (named volume, mounted at `/data`) holds the search index and the embedding model cache. For file domains those two are fully rebuildable: losing them costs a `crystalline reindex --full` (a lost volume leaves no index to destroy, so the ordinary rebuild does the whole job; the `--wipe` recovery path is for a database that is still there and will not open) and a model re-download (skipped entirely on `with-model`, since its model lives outside `/data` and is never affected by the volume), never knowledge. Three things on this volume are not rebuildable, though. If you run virtual domains, their engrams are the source of truth and live here, so back this volume up or `crystalline domain export` them to the bind mount to keep a file copy, or download the domain as a zip from Fluid's domain page (admin). `web-auth.db` holds the JSON API's accounts and sessions, which are data rather than derived state: losing it means creating every user again with `crystalline users add`. And for a domain in [review mode](../README.md#review-mode), every actor's private drafts are journaled under the daemon's state directory on this same volume rather than in the bind-mounted folder - they are unshared knowledge that exists nowhere else, so losing this volume loses them too, not merely an index to rebuild.
...
| `CRYSTALLINE_SERVICE_HTTP` | `service.http` | the HTTP endpoint carrying MCP, the JSON API and the web UI: on at `127.0.0.1:7411` by default, `false` turns it off, `true` spells the default out, or give it a `host:port` to bind instead. `serve --http` wins over it, `serve --http off` closes it - but only for that one invocation, and a `serve` whose flag disagrees with this value says so at startup |
...
| `CRYSTALLINE_SERVICE_ALLOWED_HOSTS` | `service.allowed_hosts` | comma-separated `Host` allow-list; loopback is always allowed and a single `*` allows any Host; `serve --allowed-host` wins over it. **With `auth.oauth` in effect this key does a second job**: a derived origin - what both well-known documents publish and what an authorization records as a grant's audience - is named only for a `Host` on this list, and a request arriving at any other is answered `403` rather than handed a document naming it. An `auth.oidc.redirect_uri` override is exempt, being the operator's own answer to what this instance is called. See [MCP clients over OAuth](#mcp-clients-over-oauth) |
...
| `CRYSTALLINE_AUTH_MCP` | `auth.mcp` | `true` requires every MCP connection over HTTP to authenticate with a personal MCP token: a request without a valid `Authorization: Bearer cmt_...` header is answered `401` before the transport sees it, so no session ever opens. The challenge that `401` carries follows whether OAuth is served beside it: a bare `WWW-Authenticate: Bearer` with `auth.oauth` off, and one naming `resource_metadata` where it is on - which is what an unset `auth.oauth` derives to wherever the UI is served, see [MCP clients over OAuth](#mcp-clients-over-oauth). A signed-in user issues a token in Fluid under profile > Agent access; an admin issues one for any account from the CLI. `false` (default) is the legacy open tier, where any client that can reach the port is served. Local stdio clients are unaffected: those are the machine owner. Read once when the HTTP surface starts, like `service.read_only` |
...
Review mode adds nothing else to this surface: no new transport, no new `serve` flag and no separate image variant. It is exactly the `CRYSTALLINE_DOMAIN_<NAME>_REVIEW` row above (or the equivalent `review: overlay` key in `config.yaml`) plus the `crystalline domain review` CLI verb and its REST counterpart, both already gated by the same roles as every other domain-admin action. See [Review mode](../README.md#review-mode) in the README for what it does to reads, writes and sharing.
...
`auth.mcp` (or `CRYSTALLINE_AUTH_MCP=true`) decides whether an HTTP MCP connection has to authenticate, separately from the three modes above that govern people in a browser. Off (the default) is the legacy open tier: any client that can reach the port is served, with no account behind the connection. On, every one of them presents a personal MCP token exactly like a human user, as `Authorization: Bearer cmt_...`, and a request without a valid one is answered `401` before the transport sees it, so the session never opens. The `WWW-Authenticate` challenge in that `401` is a bare `Bearer` while this instance serves no OAuth, and carries `resource_metadata="<origin>/.well-known/oauth-protected-resource"` where it does - which an unset `auth.oauth` makes the default on any instance that serves the UI, so the recipe in [MCP clients over OAuth](#mcp-clients-over-oauth) is the shape most shared instances answer with. One case reads like the first even where OAuth is on: a request whose `Host` is not on `service.allowed_hosts` gets the bare `Bearer`, since the address it named is not one this instance will publish as a resource - and the transport refuses that same request `403` a moment later anyway. Every rejected credential gets the identical answer, so nothing about the response says which token exists or why it failed. Local stdio clients are unaffected either way, since those are the machine's owner; turn this on for any instance reachable beyond a trusted network. A hosted client that speaks OAuth needs no token pasted at all: see [MCP clients over OAuth](#mcp-clients-over-oauth).
...
**Write-result nudges.** Every write, edit, move, delete or split answers with a short trailer of its own once in a while: a `---` rule followed by at most one line starting `[crystalline] `, asking the agent to propose sharing unshared work or to run maintenance - the same standing practice a Stop hook teaches, reaching here the one kind of session a hook can never nudge at its end. It is throttled per authenticated identity, at most once every four hours per nudge kind, so two agents working as different accounts are nudged on their own independent clocks; with `auth.mcp` off there is no identity to key it by and the whole instance shares one cooldown instead. A client parsing the receipt text as structured data should cut at the `---` rule rather than trying to parse the trailer itself, since that rule is the one boundary this server guarantees never appears inside the written content.
...
flowchart LR
    ...
    G -->|no token, or an invalid one| R[401 and a Bearer challenge]
...
**The origin.** Both documents, the challenge above, and every access token's audience all name the same address: this instance's origin (scheme, host, optional port, no path). Derived per request from the incoming `Host` by default, the same rule the [Enterprise SSO](#enterprise-sso) callback address uses - and overridden the same way, by `auth.oidc.redirect_uri` when that key is set, since it is the one place an operator behind a Host-rewriting proxy has already named the public address (see that key's row in [the environment table](#configure-through-environment-variables)). A deployment that rewrites `Host` and has not set that key mints tokens for an address nobody outside the machine can reach. Separately, the public host itself has to be on `service.allowed_hosts` (see [Configure through environment variables](#configure-through-environment-variables)): the MCP transport validates the `Host` header and answers `403 Forbidden` for a name it was not told to trust, and a derived origin holds to the same list - the two well-known documents and the authorize leg refuse an unlisted `Host` with their own `403` rather than publishing it, so no client is handed a resource identifier the transport then refuses, and no grant records one. With the list unset (the default) the origin rule narrows nothing and the transport keeps its own loopback-only default; a single `*` turns both off. A configured `auth.oidc.redirect_uri` is exempt, since it is the operator's answer rather than the request's - but a proxy that forwards the public hostname still needs that hostname listed, because the transport checks it either way.
...
**Starting an authorization is bounded the same way, per registration.** `GET /api/v1/oauth/authorize` carries no credential - it cannot, since there is no account yet at the moment a client sends its person here - so the bound is the `client_id` every request must name, which came from a registration and is therefore already inside the limits above. One registration may have at most 4 authorizations waiting for a decision at once, and past that its next request is answered `temporarily_unavailable` until one of its own is decided, expires, or is 30 seconds old and gives way to the newcomer. A flood therefore fills its own four slots and refuses itself, rather than filling the instance-wide pending map and refusing everyone else's consent. Behind that sits a global cap of 1000 pending authorizations, which the per-registration bound keeps an anonymous caller from reaching, and a ten-minute expiry on any request nobody decides. None of this replaces a rate limit at the proxy: if this instance is on the public internet, give `/api/v1/oauth/` a limit there like any other unauthenticated endpoint.
...
Pass `--read-only` to `serve` (or to `mcp`), set `service.read_only: true` in the config, or set `CRYSTALLINE_SERVICE_READ_ONLY=true` (the container-native spelling, see [Configure through environment variables](#configure-through-environment-variables)) to serve the content API read-only. Ten tools leave the MCP tool list on a default install, and thirteen once `github.enabled` is on, since the three collaboration verbs that write (`share_changes`, `resolve_conflict` and `withdraw_proposal`) follow the same gate. The seven write-gated ones (`write_engram`, `edit_engram`, `split_engram`, `move_engram`, `delete_engram`, plus `add_domain` and `remove_domain`, which register and unregister domains) are also refused if a client calls one by name, and so is `configure`, which writes settings. `evolve_engrams` and `provision` leave the list for a different reason and keep answering: the sweep is a pure read, but every finding it returns prescribes a mutation, and a queue of work nobody can work is noise where mutation is impossible; `provision` follows the write gate because every action but `status` writes. Meanwhile `search_engrams`, `read_engram`, `list_domains` and the rest of the read tools stay. Sync, the file watcher and embedding keep running, so the index still follows external edits such as a git pull. A read-only instance refuses a collaborative editing session too, along with every other write to the knowledge: the collab WebSocket upgrade is refused, so a Fluid page opens an engram to read but never to co-edit. The one exception is the MCP token surface (`/api/v1/me/mcp-tokens` in the API, the Agent access card in Fluid), which keeps working: what `read_only` protects is the knowledge, and a token is account state in the accounts database rather than knowledge - a read-only server with `auth.mcp` on is exactly where an agent cannot connect at all until somebody issues it one. `crystalline prompt system` and the routing block the server hands each connecting agent follow the same mode: both drop the write guidance and state that the knowledge is curated externally, and `prompt system --read-only` forces that variant on demand. The block reaches an agent by whichever channel its protocol revision uses - `initialize` for every revision before 2026-07-28, `server/discover` from then on, where the handshake no longer exists - and carries the read-only variant either way. This is the natural pairing for the git-sync setup in `compose.git-sync.yaml`, where knowledge arrives by reviewed git commits and agents only consume it. The mode is fixed for the daemon's lifetime, so an agent attaching to a running daemon gets that daemon's mode. Most operator tooling on the host (`verify`, `import`, `domain init`/`add` and `model download`) is unaffected: the boundary is that the served API is read-only, not the machine. `crystalline domain remove` is the exception and follows the mode like `crystalline config set` does, because it goes through the same engine entry point every other surface removes a domain through, and a read-only instance's registry is frozen along with its knowledge. That entry point also clears the domain's index rows, so the command needs the index open: on a Postgres deployment a removal while the database is unreachable fails rather than editing the config file alone, which is the right answer in a shared database - the rows are every instance's, and a config-only removal here would leave the domain live for everybody else. The error says as much and names the config file to edit by hand if the database is gone for good. The web UI is served in full, because reading it is all GETs: the daemon hands out the shell and the app renders itself read-only, so a read-only instance is browsable as well as searchable, and with `auth.anonymous: true` it is browsable logged out (see [Web UI from the daemon](#web-ui-from-the-daemon)).

Agents connect to the containerized daemon over its HTTP MCP endpoint, `http://localhost:7411` from the host (the image sets `CRYSTALLINE_SERVICE_HTTP=0.0.0.0:7411` by default, since a container has to bind every interface to be reachable at all - binding `127.0.0.1` inside a container is only reachable from inside that same container). The stdio `crystalline mcp` transport (see [Get started](../README.md#get-started) in the README) is for local, non-containerized processes; point a harness at the HTTP endpoint instead when Crystalline runs in a container. A browser reaches the web UI at that same address, since the image serves it built in ([Web UI from the daemon](#web-ui-from-the-daemon)). Every HTTP endpoint also answers `GET /health` with a JSON body carrying `status`, `version`, `started_by` (`serve`, `autostart` or `unknown`) and `http` (the bound address, `off`, or `unrecorded`), so a load balancer or uptime monitor can probe the daemon without an MCP handshake, and can tell a managed daemon from one a connecting agent started on its own. A script reading both surfaces should not read that pair as disagreeing: `/health` spells a closed HTTP endpoint as the string `"off"`, while the same state comes back as `null` in a `crystalline status --json`/ctl reply's own `http` field.
