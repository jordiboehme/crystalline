...
| `CRYSTALLINE_DATABASE_URL` | `database.url` | a credential when it carries a password, so `config show` and the `configure` tool render it as `(set)` and `crystalline doctor` reports the variable as set without its value |
