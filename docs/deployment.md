...
Those three modes are about the people using Fluid. The agents connecting over HTTP MCP are governed separately, by `auth.mcp`: off (the default) is the legacy open tier, where any client that can reach the port is served, and `CRYSTALLINE_AUTH_MCP=true` requires each one to present a personal MCP token as `Authorization: Bearer cmt_...`. A request without a valid token is answered `401` with `WWW-Authenticate: Bearer` before the transport sees it, so the session never opens and the refusal names where a token comes from: a signed-in user issues one in Fluid under profile > Agent access, and `crystalline users mcp-token <name>` issues one for any account from the host that holds the accounts database (`--list`, `--rotate <id>` and `--revoke <id>` manage the ones an account already holds). The token is shown exactly once, at issuance: only its hash is stored, so a lost one is revoked and replaced rather than looked up. Every rejected credential gets the identical answer, so nothing about the response says which token exists. Each request carries the header, not just the handshake: a session belongs to the account that opened it, and another account naming it - even with a valid token of its own - is answered `403` without being told whose session it is. Turn it on for any instance reachable beyond a trusted network; local stdio clients are unaffected either way, since those are the machine's owner.
