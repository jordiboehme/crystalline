...
| `CRYSTALLINE_UPSTREAM` | the Fluid image, not Crystalline itself | where Fluid's nginx forwards `/api/`, as `host:port` (default `crystalline:7411`). Read once, when nginx loads its configuration: an upstream that changes address needs the Fluid container restarted. See [Team server with Fluid](#team-server-with-fluid) |
