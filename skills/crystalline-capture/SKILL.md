...
2. Edit the old engram: set its `status` to `superseded` (or `deprecated`) with a `find_replace` targeting the frontmatter line as the old engram actually spells it, `find_text: "status: stable"` (or `"status: current"` on an engram written before that word changed) and `content: "status: superseded"`. There is no status argument on `edit_engram`; passing one is silently ignored.
