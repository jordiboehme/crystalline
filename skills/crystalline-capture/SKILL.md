...
When one fact inside an engram stops holding while the rest still does, split before you retire: move the facts that still hold into their own engram (or engrams, one per lifecycle) with `write_engram`, remove those lines from the old one with `find_replace` or `replace_section`, link the pair with `- derived_from [[Old Engram]]` on the new one and `- split_into [[New Engram]]` on the old, and only then flip the old engram's status and close its window. The retired engram then carries exactly what expired, the still-valid facts keep their unbounded validity without being re-copied into every successor, and the split marks the scoping mistake so it is not repeated. Never retire a bundle whole and re-type its surviving facts into the successor - that loses their history and their links.
