...
Always confirm with the user before calling `delete_engram`, `move_engram` or `remove_domain` - describe what will be removed or relocated and wait for a yes. Prefer setting `status` to `deprecated` or `superseded` over deleting when the history is still worth keeping; `move_engram` on a cross-domain move rewrites inbound bare links to the domain-prefixed form automatically unless `update_links` is set to `false`. A destination inside the same domain is an ordinary move too: it is how a topic cluster gets its subfolder after the fact.
