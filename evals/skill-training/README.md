...
Trains and evaluates the shipped Crystalline skills with [SkillOpt](https://github.com/microsoft/SkillOpt), a text-space optimizer that treats a skill markdown document as the trainable state of a frozen agent: rollout, reflect, bounded edits and a held-out validation gate that only accepts strict improvements. Four benchmarks: `crystalline-routing` (read side), `crystalline-capture` (write side), `crystalline-schema` (structure and conformance) and `crystalline-memory` (the consolidated skill).
...
The memory benchmark exercises `crystalline-memory`, the consolidated recall-plus-capture skill, on a stratified blend of the routing and capture items: `data/memory_split/build.py` samples both sources evenly per task type for train and val and keeps both source test splits whole, so memory numbers stay comparable to the per-skill baselines. Each item carries a `bench` field and is scored by its source benchmark's scorer, dispatched per item in `envs/crystalline_memory/rollout.py`.
