...
Trains and evaluates the shipped Crystalline skills with [SkillOpt](https://github.com/microsoft/SkillOpt), a text-space optimizer that treats a skill markdown document as the trainable state of a frozen agent: rollout, reflect, bounded edits and a held-out validation gate that only accepts strict improvements. Three benchmarks: `crystalline-routing` (read side), `crystalline-capture` (write side) and `crystalline-schema` (structure and conformance).
