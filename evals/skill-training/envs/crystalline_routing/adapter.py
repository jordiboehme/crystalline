"""SkillOpt environment adapter for crystalline routing tasks."""
...
from envs.crystalline_routing.dataloader import CrystallineRoutingDataLoader
from envs.crystalline_routing.rollout import run_batch

HARNESS_ROOT = Path(__file__).resolve().parent.parent.parent


def _resolve(path: str, default: str) -> str:
    """Resolve a possibly relative path against the harness root."""
    raw = Path(path or default)
    if not raw.is_absolute():
        raw = HARNESS_ROOT / raw
    return str(raw)


class CrystallineRoutingAdapter(EnvAdapter):
    def __init__(
        self,
        split_dir: str = "",
        data_path: str = "",
        ...
        self.dataloader = CrystallineRoutingDataLoader(
            split_dir=_resolve(split_dir, "data/routing_split"),
            data_path=data_path,
    ...
    def get_task_types(self) -> list[str]:
        ...
        for item in (
            ...
            task_type = str(item.get("task_type") or "routing")
            if task_type not in seen:
                seen.append(task_type)
        return seen or ["routing"]
