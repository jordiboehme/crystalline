...
- **Built on an open format.** The engram format extends [Google's Open Knowledge Format (OKF) v0.2](https://github.com/GoogleCloudPlatform/knowledge-catalog/tree/main/okf): plain markdown with YAML frontmatter where unknown keys are always preserved. Every engram records who wrote it and when as `generated: { by, at }`, following the OKF actor convention, and verifications and staleness bounds use the OKF spellings `verified` and `stale_after`; the earlier `timestamp`, `last_verified` and `review_after` keys are still read, so nothing written before loses its meaning. Crystalline's default `status` is the OKF word `stable`, which is also what an absent status means to an OKF consumer; `current`, the word Crystalline wrote before, means the same state and is read forever, so a search filtered on either word returns engrams carrying either and no existing file needs rewriting. Crystalline layers its routing, temporal and knowledge-graph conventions on top, so engrams stay readable by any OKF tooling and OKF documents drop into a domain with minimal ceremony.
