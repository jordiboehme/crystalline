...
- **`origin share <name> [--title <t>] [--message <m>] [--proposal <n>]`** - share local changes as a proposal the team reviews on GitHub; refuses while a conflict is unresolved so the team always reviews a clean proposal. Sharing again while a proposal is open stacks a new proposal on top of it, and `--proposal <n>` amends that layer instead.
