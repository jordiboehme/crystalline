...
/// than by sanitizing a path: the engine checks the name is one plain segment
/// (`crystalline_core::config::registration::validate_domain_name`, the rule
/// every surface shares) before it joins it under the configured root.
...
pub async fn create(
    ...
    let report = match body.mode.as_str() {
        "local" => {
            // Trimmed here, the one surface that accepts padded input; the
            // engine checks what is left, with the message every surface uses.
        ...
        "github" => {
            ...
            // and create a folder to match - exactly what the trim here
            // prevents; the engine then checks the name like every other
            // surface.
