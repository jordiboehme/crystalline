...
///
/// Policy-neutral on purpose: the hook reads no MANIFEST, and "so the team has
/// it" is true on a domain that reviews proposals and on one that commits
/// straight to its branch.
pub const SHARE_NUDGE_REASON: &str = "If that work is done, propose sharing it with share_changes so the team has it and the team's archive stays current - and wait for a yes.";
